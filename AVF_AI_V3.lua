...
function init()
	...
	if(PLAYERRACING) then
		...
		-- SetPlayerVehicle has been defered to fix race condition (check the tick function)
	end
...
-- Wait for the player's vehicle to finish initializing before assigning the player.
-- The vehicle handle can exist before Teardown has finished preparing the vehicle seat.
-- Without this check, SetPlayerVehicle may run too early and leave the player on foot.
playerCarReady = false

function tick(dt)

	-- Only attempt to place the player in the car once.
	if not playerCarReady then

		-- Wait until the selected car exists and has a valid vehicle ID.
		if aiVehicles[playerConfig.car] and aiVehicles[playerConfig.car].id ~= 0 then

			-- Assign the player to their selected race vehicle.
			SetPlayerVehicle(aiVehicles[playerConfig.car].id)

			-- Stop checking after successful assignment.
			playerCarReady = true
		end
	end
